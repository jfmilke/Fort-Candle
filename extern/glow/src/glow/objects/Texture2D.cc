...
void Texture2D::BoundTexture2D::setBorderColor(tg::color4 const& color)
{
    if (!isCurrent())
        return;
    checkValidGLOW();
...
void Texture2D::BoundTexture2D::resize(int width, int height)
{
    ...
    glTexImage2D(texture->mTarget, 0, texture->mInternalFormat, width, height, 0, format, type, nullptr);
