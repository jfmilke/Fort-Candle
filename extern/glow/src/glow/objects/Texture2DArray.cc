...
SharedTexture2DArray Texture2DArray::createStorageImmutable(Shape const& shape)
{
    ...
    tex->bind().makeStorageImmutable(size.width, size.height, size.depth, shape.format, shape.immutableMipmaps);
    return tex;
}
...
void Texture2DArray::BoundTexture2DArray::setBorderColor(tg::color4 const& color)
{
    if (!isCurrent())
        return;
    checkValidGLOW();
...
void Texture2DArray::BoundTexture2DArray::resize(int width, int height, int layers)
{
    ...
    glTexImage3D(texture->mTarget, 0, texture->mInternalFormat, width, height, layers, 0, format, type, nullptr);
