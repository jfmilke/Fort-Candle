...
bool Mesh3D::loadFromFile(const std::string& filename, bool interpolate_tangents)
{
...
glow::SharedVertexArray Mesh3D::createBasicVertexArray() const
{
    std::vector<tg::pos3> aPos;
    std::vector<tg::vec3> aNormal;
    ...
    return glow::VertexArray::create({abPos, abNormal}, eab, GL_TRIANGLES);
}
